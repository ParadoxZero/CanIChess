...
namespace pieces {
	...
	inline bool CheckBoardEdgeCollision(base::Vector2Di& new_pos)
	{
		return new_pos.x < BOARD_LEFT || new_pos.x > BOARD_RIGHT || new_pos.y < BOARD_BOTTON || new_pos.y > BOARD_TOP;
	}

	std::vector<base::Vector2Di> getPossibleMovesInDirection(base::Vector2Di current_position, std::vector<base::Vector2Di> directions, ChessBoardMatrix<ChessPiece>& map, bool ignore_friendly = false);
