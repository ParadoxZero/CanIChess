...
{
	template <class T>
	using ChessBoardMatrix = std::array<std::array<std::unique_ptr<T>, 8>, 8>;
	...
	{
	public:
		ChessPiece(ChessPieceType type, PieceColor color, IChessBoardNotifier *board = nullptr) : _type(type),
																								  _color(color),
																								  _board(board),
